...
const ProductTypes: FC<ProductTypesProps> = () => {
  ...
  const handleCopy = (selectedRows: Row<ProductType>[]) => {
  ...
  const deleteRows = async (selectedRows: Row<ProductType>[]) => {
    const deletePromises = selectedRows.map((row) => {
      //this is the mutation function
      return deleteProductType({
        variables: { productTypeId: row.original.productTypeId },
      });
    });
  ...
  const handleDelete = async (selectedRows: Row<ProductType>[]) => {
    try {
      toast({
  ...
  const handleEdit = (selectedRows: Row<ProductType>[]) => {
    navigate(`/edit-product-type/${selectedRows[0].original.productTypeId}`);
  };
