...
import ledgerAccountList from '@/components/ledger-account-list/ledger-accounts.json';
...
const LedgerAccounts: FC<LedgerAccountsProps> = () => {
    const ledgerAccounts: LedgerAccount[] = ledgerAccountList as LedgerAccount[];
    const location = useLocation();
    const navigate = useNavigate();
    const from = location.state?.from || { pathname: "/administration/ledger-management/ledger-accounts/new-ledger-account" };
    return (
      <div>
        <div className="mx-4">
          <div className="pt-2">
          ...
          <div>
            {ledgerAccounts && (
              <DataTable columns={columns} data={ledgerAccounts} />
            )}
          </div>
        </div>
      </div>
